...
package com.grantingersoll.opengrok.management;
...
import com.grantingersoll.opengrok.Info;
import com.grantingersoll.opengrok.logger.LoggerFactory;
import com.grantingersoll.opengrok.logger.LoggerUtil;
import com.grantingersoll.opengrok.Info;
import com.grantingersoll.opengrok.logger.LoggerFactory;
import com.grantingersoll.opengrok.logger.LoggerUtil;
...
final public class OGAgent {
    ...
    public static void main(final String args[]) {
        ...
        if (success) {
            setIfNotSet(props, Constants.LOG_PATH, uri.getPath() + "/log");

            setIfNotSet(props, Constants.CONFIG_FILE,
                    uri.getPath() + "/etc/configuration.xml");

            setIfNotSet(props, Constants.JMX_HOST, uri.getHost());

            setIfNotSet(props, Constants.JMX_PORT, String.valueOf(uri.getPort()));

            setIfNotSet(props, Constants.RMI_PORT, String.valueOf(uri.getPort() + 1));

            setIfNotSet(props, Constants.JMX_URL,
                    "service:jmx:rmi://" + props.getProperty(Constants.JMX_HOST) + ":" +
                    props.getProperty(Constants.JMX_PORT) + "/jndi/rmi://" +
                    props.getProperty(Constants.JMX_HOST) + ":" +
                    props.getProperty(Constants.RMI_PORT) + "/opengrok");
    ...
    public void runOGA() throws MalformedURLException, IOException, JMException {
        ...
        String urlString = props.getProperty(Constants.JMX_URL);
        ...
        if (url.getProtocol().equals(Constants.RMI_PROTOCOL) &&
                Boolean.parseBoolean(props.getProperty(Constants.RMI_START))) {
            int rmiport = Integer.parseInt(props.getProperty(Constants.RMI_PORT));
    ...
    private void createIndexTimer(Properties properties) throws JMException {
        ...
        boolean enabled = Boolean.parseBoolean(properties.getProperty("com.grantingersoll.opengrok.management.indexer.enabled"));
        int period = Integer.parseInt(properties.getProperty("com.grantingersoll.opengrok.management.indexer.sleeptime"));
        ...
        // Add index notification to timer (read from com.grantingersoll.opengrok.management.indexer.sleeptime property).
    ...
    private static boolean createLogger(Properties props) {
        boolean ret = true;
        String OGAlogpath = props.getProperty("com.grantingersoll.opengrok.management.logging.path");

        Level loglevel = null;
        try {
            loglevel = Level.parse(props.getProperty("com.grantingersoll.opengrok.management.logging.filelevel"));
        } catch (Exception exll) {
            loglevel = Level.FINE;
        }
        Level consoleloglevel = null;
        try {
            consoleloglevel = Level.parse(props.getProperty("com.grantingersoll.opengrok.management.logging.consolelevel"));
